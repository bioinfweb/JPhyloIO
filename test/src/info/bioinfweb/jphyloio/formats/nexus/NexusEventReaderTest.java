...
public class NexusEventReaderTest implements NexusConstants, ReadWriteConstants, NewickConstants, W3CXSConstants {
	...
	public void testReadingMatrix_noLineBreak() throws Exception {
		...
		try {
			...
			String idDE = assertLabeledIDEvent(EventContentType.OTU, null, "D E", reader).getID();
			assertEndEvent(EventContentType.OTU, reader);
			assertLabeledIDEvent(EventContentType.OTU, null, "F", reader).getID();
			assertEndEvent(EventContentType.OTU, reader);
			assertEndEvent(EventContentType.OTU_LIST, reader);
	...
	@Test
	public void testReadingMatrix_additionalLineBreaks() throws Exception {
		NexusEventReader reader = new NexusEventReader(new File("data/Nexus/Matrix_additionalLineBreaks.nex"), new ReadWriteParameterMap());
		try {
			assertEventType(EventContentType.DOCUMENT, EventTopologyType.START, reader);
			String otusID = assertLabeledIDEvent(EventContentType.OTU_LIST, null, null, reader).getID();
