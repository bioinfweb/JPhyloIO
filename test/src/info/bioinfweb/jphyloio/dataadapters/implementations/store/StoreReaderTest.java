...
public class StoreReaderTest {
	...
	@Test
	public void test_readTreeNetwork_newIDs() throws IOException {
		NewickEventReader reader = new NewickEventReader(new File("data/Newick/SimpleTree.nwk"), new ReadWriteParameterMap());
		try {
			while (!reader.peek().getType().equals(EventContentType.TREE, EventTopologyType.START)) {
				reader.next();
			}
			...
			Iterator<String> iterator = tree.getNodes(null).getIDIterator(null);
			while (iterator.hasNext()) {
				assertEquals(ReadWriteConstants.DEFAULT_GENERAL_ID_PREFIX + idManager.createNewID(), iterator.next());
			}
			
			iterator = tree.getEdges(null).getIDIterator(null);
			while (iterator.hasNext()) {
				assertEquals(ReadWriteConstants.DEFAULT_GENERAL_ID_PREFIX + idManager.createNewID(), iterator.next());
			}
		}
